...
Section measure.
  ...
  Class Measure (M : MeasureFun) : Prop :=
    { measureEmptyAx : μ (@empty X) = Rpluszero;
      measureAddAx : forall C : sequence (set X),
          countable_disjoint C ->
          μ (countable_union C) = Rplus_sequence_sum (fun i => μ (C i)) }.
End measure.
...
Section lebesgue.
  ...
  Definition lebesgue_measure (E : set R) : Rplus :=
    RplusInfimum (fun x =>
                 ...
                 exists f : nat -> qI,
                   ...
                   x ~~ Rplus_sequence_sum (map_sequence qI_length f)).

  (** The Lebesgue measure of the empty set is zero. *)
  Lemma lebesque_empty : lebesgue_measure (@empty R) = Rpluszero.
  Proof.
    destruct (lebesgue_measure _) as [x Hx] eqn:H0; inversion H0 as [H1].
    destruct Rpluszero as [x0 Hx0] eqn:H2; inversion H2 as [H3].
    assert (H4: x = x0).
        ...
        + split.
          ...
          * intros y Hy; simpl in *.
            ...
            -- split; auto; intros ? [? [[i ?] ?]]; subst.
               ...
               rewrite Rplus_of_Q_0; rewrite sum_list_const_0; auto.
            -- exists (y / (2#1)); split.
               ++ split.
                  ** intros A [r [HA ?]]; subst.
  ...
  Lemma lebesque_additivity :
    forall C : sequence (set R),
      ...
      Rplus_sequence_sum (fun i => lebesgue_measure (C i)).
  Proof. Admitted.
